...
public class Reflector {
  ...
  /**
   * 属性对应的 setting 方法的映射。在 <3> 处初始化，详细解析，见 addSetMethods(clazz);
   * key 为属性名称
   * value 为 Invoker 对象
   */
  private final Map<String, Invoker> setMethods = new HashMap<String, Invoker>();
  /**
   * 属性对应的 getting 方法的映射。在 <2> 处初始化，详细解析，见 addGetMethods(clazz);
   * key 为属性名称
   * value 为 Invoker 对象
   */
  private final Map<String, Invoker> getMethods = new HashMap<String, Invoker>();
  /**
   * 属性对应的 setting 方法的方法参数类型的映射。{@link #setMethods} 在 <3> 处初始化，详细解析，见 addSetMethods(clazz);
   * key 为属性名称
   * value 为方法参数类型
   */
  private final Map<String, Class<?>> setTypes = new HashMap<String, Class<?>>();
  /**
   * 属性对应的 getting 方法的返回值类型的映射。{@link #getMethods} 在 <2> 处初始化，详细解析，见 addGetMethods(clazz);
   * key 为属性名称
   * value 为返回值的类型
   */
  private final Map<String, Class<?>> getTypes = new HashMap<String, Class<?>>();
  /**
   * 默认构造方法，默认无参构造方法。在 <1> 处初始化，详细解析，见 addDefaultConstructor(clazz)
   */
  ...
  private void addGetMethods(Class<?> cls) {
    // <1> 属性与其 getting 方法的映射。因为父类和子类都可能定义了相同属性的 getting 方法，所以 VALUE 会是个数组
  ...
  /**
   * 解决 getting 冲突。最终，一个属性，只保留一个对应的方法
   * @param conflictingGetters 属性与其 getting 方法的映射
   */
  private void resolveGetterConflicts(Map<String, List<Method>> conflictingGetters) {
    // 遍历每个属性，查找其最匹配的方法。因为子类可以重写父类的方法，所以一个属性，可能对应多个 getting 方法
    for (Entry<String, List<Method>> entry : conflictingGetters.entrySet()) {
      ...
      for (Method candidate : entry.getValue()) {
        ...
        // <1.1> 符合选择子类。因为子类可以修改放大返回值。例如，父类的一个方法的返回值为 List ，子类对该方法的返回值可以重写为 ArrayList
  ...
  /**
   * 通过遍历 setting 方法，初始化 setMethods 和 setTypes
   * @param cls
   */
  ...
  /**
   * 解决 setting 冲突方法
   * @param conflictingSetters 属性与其 setting 方法的映射
   */
  private void resolveSetterConflicts(Map<String, List<Method>> conflictingSetters) {
    // 遍历每个属性，查找其最匹配的方法。因为子类可以重写父类的方法，所以一个属性，可能对应多个 setting 方法
  ...
  private Method pickBetterSetter(Method setter1, Method setter2, String property) {
    ...
    // isAssignableFrom() 方法 判断当前的Class对象所表示的类，是不是参数中传递的Class对象所表示的类的父类，超接口，或者是相同的类型。是则返回true，否则返回false
  ...
  private void addFields(Class<?> clazz) {
    ...
    for (Field field : fields) {
      ...
      if (field.isAccessible()) {
        // <1> 若 fieldName 不存在于 setMethods 中，则调用 #addSetField(Field field) 方法，添加到 setMethods 和 setTypes 中
        ...
        // <2> 若 fieldName 不存在 getMethods 中，则调用 #addGetField(Field field) 方法，添加到 getMethods 和 getTypes 中
   ...
   * 获得所有方法
   *
   * 该方法返回一个数组，其中包含所有和任何超类中声明的所有方法。
   * 我们使用这个方法，而不是更简单的Class.getMethods()、
   * 因为我们也想寻找私有方法。
   *
   * @param 类
   * @return 一个包含该类中所有方法的数组
   */
  private Method[] getClassMethods(Class<?> cls) {
    ...
    while (currentClass != null && currentClass != Object.class) {
      ...
      // <2> 记录接口中定义的方法（我们还需要寻找接口方法，因为这个类可能是抽象的）
